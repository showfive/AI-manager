use ai_manager_shared::{ServiceId, Result, ServiceHealth};
...
impl ServiceManager {
    ...
    pub async fn restart_service(&mut self, service_id: &ServiceId) -> Result<()> {
        ...
        warn!("Service restart not fully implemented - would restart '{}' here", service_id);
    ...
    fn calculate_restart_delay(&self, restart_count: u32) -> Duration {
        ...
        let multiplier = self.restart_policy.backoff_multiplier.powi(restart_count as i32);
    ...
    pub fn should_restart_service(&self, service_id: &ServiceId, restart_count: u32) -> bool {
...
mod tests {
    ...
    async fn test_service_lifecycle() {
        ...
        let result = manager.start_service("test-service".to_string(), || async {
        ...
        let status = manager.get_service_status("test-service").await;
        assert!(status.is_some());
        
        // Stop service
        let result = manager.stop_service("test-service").await;
        assert!(result.is_ok());
