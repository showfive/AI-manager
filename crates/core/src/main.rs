use ai_manager_core::{
    event_bus::EventBus,
    service_manager::{ServiceManager, RestartPolicy},
    config::ConfigManager,
    handlers::{UserInputHandler, LLMResponseHandler, SystemEventHandler},
};
use ai_manager_shared::{ServiceMessage, Result, CORE_SERVICE_ID};
use std::sync::Arc;
...
async fn main() -> Result<()> {
    ...
    let mut service_manager = ServiceManager::new(event_bus.clone())
    ...
    match service_manager.start_service(CORE_SERVICE_ID.to_string(), core_service_task).await {
...
impl CoreService {
    ...
    async fn start(&mut self) -> Result<()> {
        ...
        let (_tx, mut rx) = self.event_bus.register_service(CORE_SERVICE_ID.to_string()).await?;
        ...
        while let Some(message) = rx.recv().await {
            ...
            let result = match &message {
                ...
                ServiceMessage::LLMResponse { .. } => {
                    llm_response_handler.handle_llm_response(message.clone()).await
                }
