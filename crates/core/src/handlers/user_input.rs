use ai_manager_shared::{ServiceMessage, ResponseType, SystemError, Result, LLM_SERVICE_ID};
...
impl UserInputHandler {
    ...
    pub async fn handle_user_input(&self, user_input: ServiceMessage) -> Result<()> {
        if let ServiceMessage::UserInput { content, timestamp, user_id } = user_input {
            ...
            self.event_bus.route_message(thinking_response, None).await?;
            ...
            self.event_bus.route_message(llm_request, Some(LLM_SERVICE_ID.to_string())).await?;
            
            debug!("User input routed to LLM service");
            Ok(())
        } else {
            error!("Invalid message type for user input handler");
            Err(SystemError::InvalidInput("Expected UserInput message".to_string()))
        }
    }
...
mod tests {
    ...
    async fn test_user_input_handler() {
        ...
        // Register LLM service to receive messages
        let _llm_service = event_bus.register_service(LLM_SERVICE_ID.to_string()).await.unwrap();
