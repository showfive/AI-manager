use ai_manager_shared::{ServiceMessage, ResponseType, SystemError, Result, DATA_SERVICE_ID, UI_SERVICE_ID, Message, MessageRole};
...
impl LLMResponseHandler {
    ...
    pub async fn handle_llm_response(&self, llm_response: ServiceMessage) -> Result<()> {
        if let ServiceMessage::LLMResponse { content, usage, request_id } = llm_response {
            info!("Processing LLM response for request {}", request_id);
            debug!("LLM response content: {} (tokens: {})", content, usage.total_tokens);
            ...
            self.event_bus.route_message(ui_response, Some(UI_SERVICE_ID.to_string())).await?;
            ...
            self.event_bus.route_message(store_request, Some(DATA_SERVICE_ID.to_string())).await?;
            
            info!("LLM response processed and routed successfully");
            Ok(())
        } else {
            error!("Invalid message type for LLM response handler");
            Err(SystemError::InvalidInput("Expected LLMResponse message".to_string()))
        }
    }
    
    /// Handle LLM errors
    pub async fn handle_llm_error(&self, provider: &str, error_message: &str, request_id: Uuid) -> Result<()> {
        error!("LLM error from provider '{}' for request {}: {}", provider, request_id, error_message);
        
        // Create error response for UI
        let error_response = ServiceMessage::SystemResponse {
            content: format!("Sorry, I encountered an error while processing your request: {}", error_message),
            message_type: ResponseType::Error,
            timestamp: Utc::now(),
        };
        
        // Route error to UI
        self.event_bus.route_message(error_response, Some(UI_SERVICE_ID.to_string())).await?;
...
mod tests {
    ...
    async fn test_llm_response_handler() {
        ...
        let _ui_service = event_bus.register_service(UI_SERVICE_ID.to_string()).await.unwrap();
        let _data_service = event_bus.register_service(DATA_SERVICE_ID.to_string()).await.unwrap();
    ...
    async fn test_llm_error_handling() {
        ...
        let result = handler.handle_llm_error("openai", "API rate limit exceeded", Uuid::new_v4()).await;
        assert!(result.is_ok());
    }
}