use ai_manager_shared::{ServiceMessage, SystemEvent, SystemError, Result, ServiceId, MESSAGE_QUEUE_CAPACITY, BROADCAST_CHANNEL_CAPACITY};
...
impl EventBus {
    ...
    pub async fn register_service(&self, service_id: ServiceId) -> Result<(MessageSender, MessageReceiver)> {
    ...
    pub async fn unregister_service(&self, service_id: &ServiceId) -> Result<()> {
        ...
        let event = SystemEvent::ServiceStopped { service_id: service_id.clone() };
        self.broadcast_event(event).await;
    ...
    pub async fn route_message(&self, message: ServiceMessage, target_service: Option<ServiceId>) -> Result<()> {
            ...
            Some(tx) => {
                ...
                if let Err(e) = tx.send(message.clone()).await {
                    ...
                    return Err(SystemError::ServiceCommunication(
                        format!("Failed to send message to service '{}': {}", target, e)
                    ));
                }
    ...
    fn determine_target_service(&self, message: &ServiceMessage) -> Result<ServiceId> {
        ...
        let target = match message {
            ...
            ServiceMessage::EmailProcess { .. } => EXTERNAL_SERVICE_ID,
...
mod tests {
    ...
    async fn test_message_routing() {
        ...
        bus.route_message(message.clone(), Some(service_id)).await.unwrap();
