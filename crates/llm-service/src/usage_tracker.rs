...
impl UsageTracker {
    ...
    pub async fn calculate_cost(
        ...
        if let Some(pricing_info) = pricing.get(&model_key) {
            let prompt_cost =
                (usage.prompt_tokens as f64 / 1000.0) * pricing_info.prompt_price_per_1k;
            let completion_cost =
                (usage.completion_tokens as f64 / 1000.0) * pricing_info.completion_price_per_1k;
    ...
    pub async fn get_stats(&self) -> UsageStats {
        ...
        for provider_stats in stats.by_provider.values_mut() {
            if provider_stats.requests > 0 {
                provider_stats.average_tokens_per_request =
                    provider_stats.tokens as f64 / provider_stats.requests as f64;
            }
        }
...
mod tests {
    ...
    async fn test_usage_tracking() {
        ...
        tracker
            .record_usage("openai", "gpt-3.5-turbo", &usage1)
            .await;
        ...
        tracker
            .record_usage("claude", "claude-3-haiku-20240307", &usage2)
            .await;
    ...
    async fn test_record_filtering() {
        ...
        tracker
            .record_usage("openai", "gpt-3.5-turbo", &usage)
            .await;
